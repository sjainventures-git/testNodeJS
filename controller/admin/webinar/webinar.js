...
const tb_courses = require('../../../model/tbl_webinar.js'); 
const tbl_webinar_session = require('../../../model/tbl_webinar_session.js'); 
...
// delete course	
    ...
    try {
			...
			// update will be here
				if(isallgood==1)
					{
						var myquery 	=	{ webinarid: webinarid };
						var newvalues 	= 	{
												is_deleted: 0
											};
...
// create session on the webinars
	router.post('/manageSession', async(req, res) => {
		var message = "Something Went Wrong, Please try again later...";
		var isallgood = 1;
		...
		// Works will be Here
				try {
					...
					var values       =   rycommon.check_if_value(req,'values',[]);
					var webinarid    =   values[0].webinarid;
					
					//  isallgood = 0;

				//	console.log(newvalues.targetaud);
			if(webinarid !== 0 && webinarid !== '0' && webinarid !== null)
				{
					
					// for delete the ols session here
						await tbl_webinar_session.deleteMany({ webinarid: webinarid }).then(function(){
					...
					// insert will be here
						if(isallgood==1)
							{
								console.log("Data Started adding");
								values.forEach(element => {
									var newvalues	=	{
											// _id 		:	element.index,
											index 		:	element.index,
											s_date 		:	element.s_date,
											s_time 		:	element.s_time,
											webinarid 	:	webinarid,
										};

									const data = new tbl_webinar_session(newvalues);							   
											data.save()
											.then(item => {
												message  =   "Webinars Session Created Successfully.";
											})
											.catch(err => {
					...
					return res.status(200).json({status: isallgood, message: message, values: values}) 
...
var CurrentWebinarId = 0;
	...
	if(category == 0 || category == '0' || category == '') {
		...
		//	console.log(newvalues.targetaud);
	if(webinarid===0 || webinarid==='0' || webinarid==='create')
		{
			// insert will be here
				if(isallgood==1)
					{
					  ...
					  await data.save()
						.then(item => {
							if (isadmin===0) {
								...
								CurrentWebinarId = item.webinarid;
			...
			// update will be here
                    ...
                    return res.status(200).json({status: isallgood, message: message, currentwebinarid: CurrentWebinarId}) 
		...
		//	console.log(filterarray);
			let data =	[];
				...
				if(RequiredValueOnly != '0' && RequiredValueOnly != '0' && RequiredValueOnly != '' && RequiredValueOnly === 1) {
					data = await tb_courses.find(filterarray).sort(orderby).skip(PageNumber*recordsPerPage).limit(recordsPerPage).select(' webinarid meta_keyword title meta_description courseslug banner');
				} else {
					data = await tb_courses.find(filterarray).sort(orderby).skip(PageNumber*recordsPerPage).limit(recordsPerPage);
				}
		...
		try
			{
				...
				filterarray.status		=	1;
				filterarray.is_deleted	=	1;
				data = await tb_courses.find(filterarray).sort(orderby).skip(0).limit(500).select(' webinarid title  courseslug banner updatedAt');
...
router.post('/getOne', async(req, res) => {
	...
	try
		{
			const data = await tb_courses.findOne({webinarid: webinarid});
			...
			const sessions = await tbl_webinar_session.find({webinarid: webinarid}).sort(orderby);
			
			res.status(200).json({status:1, message:'Detail of Blog', data: data, sessions: sessions, category: category});
		}
	catch(error)
		{
...
router.post('/getOnebySlug', async(req, res) => {
	...
	try
		{
			const data = await tb_courses.findOne({courseslug: courseslug});
			const category = await tb_category.findOne({catid: data.category}).select("catid title");	
			...
			const orderby 	=  { index: 1 };
			const sessions = await tbl_webinar_session.find({webinarid: data.webinarid}).sort(orderby);
			
			res.status(200).json({status:1, message:'Detail of Blog', data: data, category: category, sessions: sessions});
		}
	catch(error)
		{
			res.status(500).json({status:0, message: error.message});
								...
								if (userAuth.username === 'jitesh' && userAuth.password === 'Pace2012#')
									{
										...
										data = await tb_courses.find(filterarray).sort(orderby).skip(Number(page)*limit).limit(limit).select('-_id webinarid validityin validity view courseslug level title meta_description rating banner updatedAt');
										const count = await tb_courses.count(filterarray);
...
data = await tb_category.find(filterarray).sort(orderby).skip(0).limit(1000).select(' -_id catid  title image updatedAt ');
	...
	res.status(200).json({
		...
		faqs,
									...
									res.status(200).json({status:1, message:'No Course Data with your keyword Data.', data: data });
								}
