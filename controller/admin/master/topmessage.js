...
// update status of admin users	
	...
	var isallgood = 1; 
    try {
            var topmessageid   =	rycommon.check_if_value(req,'topmessageid',0);
            var status	=   rycommon.check_if_value(req,'status',0);
			// update will be here	
				if(isallgood==1)
					{
							var myquery 	=	{ topmessageid: topmessageid };
							var newvalues 	= 	{
													status: status
												};
						  tbl_topmessage.updateOne(myquery, newvalues, function(err, res) {
...
var isallgood = 1;
    ...
    try {
            ...
            var topmessageid       	=   req.body.topmessageid;      
	...
	if ( topmessageid===0 || topmessageid==='NEW' )
		{
			// insert will be here
				if(isallgood==1)
					{
						const data = new tbl_topmessage(the_data);
...
router.post('/getAll', async(req, res) => {
	...
	try
		{	
			const data = await tbl_topmessage.find(filterarray).sort(orderby).skip(PageNumber*recordsPerPage).limit(recordsPerPage);
			const count = await tbl_topmessage.count(filterarray);
...
router.post('/getone', async(req, res) => {
	...
	try
		{
			const data = await tbl_topmessage.findOne({topmessageid: topmessageid});
			res.status(200).json({status:1, message:'Detail of Category', data: data, topmessageid: topmessageid});
		}
	catch(error)
		{
