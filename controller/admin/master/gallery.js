...
const tbl_gallery = require('../../../model/tbl_gallery.js'); 
...
// update status of admin users	
	...
	var isallgood = 1; 
    try {
            var galleryid   =	rycommon.check_if_value(req,'galleryid',0);
            var status	=   rycommon.check_if_value(req,'status',0);
			// update will be here	
				if(isallgood==1)
					{
							var myquery 	=	{ galleryid: galleryid };
							var newvalues 	= 	{
													status: status
												};
						  tbl_gallery.updateOne(myquery, newvalues, function(err, res) {
...
var isallgood = 1;
	...
	if ( galleryid===0 || galleryid==='NEW' )
		{
			// insert will be here
				if(isallgood==1)
					{
						const data = new tbl_gallery(the_data);
...
router.post('/getAll', async(req, res) => {
	...
	try
		{	
			const data = await tbl_gallery.find(filterarray).sort(orderby).skip(PageNumber*recordsPerPage).limit(recordsPerPage);
			const count = await tbl_gallery.count(filterarray);
...
router.post('/getone', async(req, res) => {
	...
	try
		{
			const data = await tbl_gallery.findOne({galleryid: galleryid});
			res.status(200).json({status:1, message:'Detail of Category', data: data, galleryid: galleryid});
		}
	catch(error)
		{
