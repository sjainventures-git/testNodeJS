...
// update status of admin users	
	...
	var isallgood = 1; 
    try {
            var renownedclientsid   =	rycommon.check_if_value(req,'renownedclientsid',0);
            var status	=   rycommon.check_if_value(req,'status',0);
			// update will be here	
				if(isallgood==1)
					{
							var myquery 	=	{ renownedclientsid: renownedclientsid };
							var newvalues 	= 	{
													status: status
												};
						  tbl_renownedclients.updateOne(myquery, newvalues, function(err, res) {
...
var isallgood = 1;
    ...
    try {
            ...
            var renownedclientsid       	=   req.body.renownedclientsid;      
	...
	if ( renownedclientsid===0 || renownedclientsid==='NEW' )
		{
			// insert will be here
				if(isallgood==1)
					{
						const data = new tbl_renownedclients(the_data);
...
router.post('/getAll', async(req, res) => {
	...
	try
		{	
			const data = await tbl_renownedclients.find(filterarray).sort(orderby).skip(PageNumber*recordsPerPage).limit(recordsPerPage);
			const count = await tbl_renownedclients.count(filterarray);
...
router.post('/getone', async(req, res) => {
	var renownedclientsid       =   rycommon.check_if_value(req,'renownedclientsid',0);
	console.log(renownedclientsid);
	try
		{
			const data = await tbl_renownedclients.findOne({renownedclientsid: renownedclientsid});
			res.status(200).json({status:1, message:'Detail of Category', data: data, renownedclientsid: renownedclientsid});
		}
	catch(error)
		{
